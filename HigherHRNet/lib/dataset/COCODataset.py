...
class CocoDataset(Dataset):
    ...
    def __init__(self, root, dataset, data_format, transform=None, target_transform=None):
        self.name = 'COCO'
        self.root = root
        self.dataset = dataset  # 训练集 ？ 验证集 ？ 测试集
        self.data_format = data_format # 数据格式：jpg ? zip
        self.coco = COCO('../../' + self._get_anno_file_name())  # 数据集的完整路径
    ...
    def _get_anno_file_name(self):  # 返回完整的数据集的路径
        # example: root/annotations/person_keypoints_tran2017.json
        # image_info_test-dev2017.json
        if 'test' in self.dataset:
            return os.path.join(self.root, 'annotations', 'image_info_{}.json'.format(self.dataset))
        else:
            path = os.path.join(self.root, 'annotations', 'person_keypoints_{}.json'.format(self.dataset))
            path = path.replace('\\', '/')
            return path
