...
def build_dataset(cfg, is_train):
    # 设置图像增强相关操作（随机旋转[-30, 30]，缩放[0.75, 1,.5]，水平翻转，归一化）
    transforms = build_transforms(cfg, is_train)

    if cfg.DATASET.SCALE_AWARE_SIGMA:   # False
        _HeatmapGenerator = ScaleAwareHeatmapGenerator
    else:
        _HeatmapGenerator = HeatmapGenerator

    heatmap_generator = [
        _HeatmapGenerator(    # 热图生成器
            output_size, cfg.DATASET.NUM_JOINTS, cfg.DATASET.SIGMA
        ) for output_size in cfg.DATASET.OUTPUT_SIZE    # 生成两个heatmap
        # output_size = 128, 256
        # cfg.DATASET.NUM_JOINTS = 17
        # cfg.DATASET.SIGMA = 2
    ]
    joints_generator = [
        JointsGenerator(    # 关键点生成器
    ...
    dataset = eval(cfg.DATASET.DATASET)(cfg,
