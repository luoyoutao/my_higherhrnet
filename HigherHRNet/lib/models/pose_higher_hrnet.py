            ...
            for j in range(num_branches):
                if j > i:
                    fuse_layer.append(nn.Sequential(
                        nn.Conv2d(num_inchannels[j], num_inchannels[i], (1, 1), (1, 1), 0, bias=False),
                    ...
                    for k in range(i-j):
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(nn.Sequential(
                                nn.Conv2d(num_inchannels[j], num_outchannels_conv3x3, (3, 3), (2, 2), 1, bias=False),
...
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        # stem net
        self.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(2, 2), padding=1, bias=False)
    ...
    def _make_final_layers(self, cfg, input_channels):
        ...
        final_layers.append(nn.Conv2d(
            ...
            kernel_size=extra.FINAL_CONV_KERNEL, stride=(1, 1),
    ...
    def _make_deconv_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            ...
            layers.append(nn.Sequential(
                nn.ConvTranspose2d(
                    in_channels=input_channels, out_channels=output_channels,
                    kernel_size=deconv_kernel, stride=(2, 2), padding=padding,
    ...
    def _make_transition_layer(self, num_channels_pre_layer, num_channels_cur_layer):
        # [256]   [32, 64]         [32, 64]   [32, 64, 128]        [32, 64, 128]   [32, 64, 128, 256]
        num_branches_pre = len(num_channels_pre_layer)     # 前一级的层数
        num_branches_cur = len(num_channels_cur_layer)     # 后一级的层数

        transition_layers = []
        for i in range(num_branches_cur):  # 后一级的第某层 2， 3， 4
            if i < num_branches_pre:         # 当前层数小于前一级的层数
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(nn.Sequential(
                        nn.Conv2d(num_channels_pre_layer[i],
                                  num_channels_cur_layer[i], (3, 3), (1, 1), 1, bias=False),
                ...
                for j in range(i+1-num_branches_pre):
                    inchannels = num_channels_pre_layer[-1]    # 输入通道数为前一级的最后一层的通道数
                    outchannels = num_channels_cur_layer[i] \
                        if j == i-num_branches_pre else inchannels
                    conv3x3s.append(nn.Sequential( nn.Conv2d(
                            inchannels, outchannels, (3, 3), (2, 2), 1, bias=False),
                        nn.BatchNorm2d(outchannels), nn.ReLU(inplace=True)))
                transition_layers.append(nn.Sequential(*conv3x3s))

        return nn.ModuleList(transition_layers)

    # stem层的后四个Bottleneck层
    def _make_layer(self, block, planes, blocks, stride=1):     # Bottleneck, 64, 4
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:   # 需要下采样或者通道扩张
            downsample = nn.Sequential(
                nn.Conv2d(in_channels=self.inplanes, out_channels=planes * block.expansion,
                          kernel_size=(1, 1), stride=(stride, stride), bias=False),   # 扩张通道64 * 4
                nn.BatchNorm2d(num_features=planes * block.expansion, momentum=BN_MOMENTUM))

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):    # 1, 2, 3
